terraform {
  backend "s3" {
    ...
    region = "eu-west-2"
    profile = "profusion_deployment_devops_iam"
    key = "foundations/terraform.state"
    bucket = "profusion-terraform-bucket"
  }
}
provider "aws" {
  alias = "root"
...
module "profusion_developer_policy" {
  source  = "terraform-aws-modules/iam/aws//modules/iam-policy"
  version = "~> 4"
  name        = "profusion_dev"
  path        = "/profusion/"
  description = "Generic policy for Profusion Engineers."
  policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
module "profusion_developer_policy_s3" {
  source  = "terraform-aws-modules/iam/aws//modules/iam-policy"
  version = "~> 4"
  name        = "profusion_dev_s3"
  path        = "/profusion/"
  description = "Generic policy for Profusion Engineers for s3 specifically."
  policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
      ...
      "Effect": "Allow",
       "Resource": [
          "arn:aws:s3:::profusion-dataengineeringsandbox-terraform/*",
          "arn:aws:s3:::profusion-dataengineeringsandbox-terraform"
      ]
    }
  ]
...
#
module "profusion_developer_assumable_role" {
  ...
  create_role = true
  role_name         = "profusion_dev"
  role_requires_mfa = false
  custom_role_policy_arns = [
    module.profusion_developer_policy.arn,
    module.profusion_developer_policy_s3.arn
  ]
